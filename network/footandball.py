...
from data.augmentation import BALL_LABEL, BALL_BBOX_SIZE
...
class FootAndBall(nn.Module):
    ...
    def detect(self, ball_feature_map) -> List[Dict[str, torch.Tensor]]:
        """
        args: ball_feature_map: (B, C=2, H, W) tensor

        returns: list of dicts, each dict contains:
        {'boxes': tensor([[559.5000, 243.5000, 579.5000, 263.5000]], device='cuda:0'),
        'labels': tensor([1]),
        'scores': tensor([0.9997], device='cuda:0'),
        'ball_feature_map': very large tensor, device='cuda:0')}
        """
        # downscale_factor: downscaling factor of the confidence map versus an original image
...
def build_footandball_detector1(phase='train', max_ball_detections=2, ball_threshold=0.7):
    # phase: 'train' or 'eval'
    assert phase in ['train', 'eval', 'detect']
...
def model_factory(model_name, phase, max_ball_detections=2, ball_threshold=0.7):
    if model_name == 'fb1':
