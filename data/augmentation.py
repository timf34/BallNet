...
# Note: I have changed some of the transformations to account for when there are no boxes or labels (I've added if else
# statements to two of the functions)
...
def heatmap2image(tensor, channel=1):
    ...
    # Note: that in the case of 3 channels, we take the first channel (ie there must be two channels,
    # where n=0 is the background, and n=1 is the object channel)

    # Note that we will also now account for the case that dim() = 4 (ie we have a batch of images)
    # ie [batch_size, h, w, n_channels] or also [batch_size, n_channels, h, w] (assuming n_channels=2)

    # TODO: this is all kinda messy and I need to annotate + clean it up

    # Check if list
    if isinstance(tensor, list):
        tensor = tensor[0]

    assert tensor.dim() in [2, 3, 4]

    if tensor.dim() == 4:
        if tensor.shape[1] == 2:
            tensor = tensor[0, 1, :, :]
        else:
            tensor = tensor[0, :, :, channel]

    if tensor.dim() == 3:
        if tensor.shape[0] == 1:
            tensor = tensor[0, :, :]
        elif tensor.shape[2] == 1:  # Messy but: this is for cases where tensor shape is [h, w, 1] (if we tried to access channel=1), it would be out of bounds)
            tensor = tensor[:, :, 0]
        else:
            tensor = tensor[:, :, channel]


    # Note: that I had to separate the two lines below - multiplying by 255 first as otherwise everything defaults to 0
    image = tensor.cpu().detach().numpy() * 255
...
def apply_transform_and_clip(boxes, labels, M, shape):
    ...
    :return:
    """
    # print('len boxes: ', len(boxes), 'len labels', len(labels))
    # print('shape', boxes.shape)
    assert len(boxes) == len(labels)
    if len(boxes) == 0:
        return clip(boxes, labels, shape)
    else:
...
class RandomAffine:
    ...
    def __init__(self, degrees=5, translate=None, scale=(0.8, 1.2), shear=None, p_hflip=0.5):
        ...
        if shear is not None:
            if isinstance(shear, numbers.Number):
                ...
                # print("test shear 3", self.shear)
                # X-Axis shear with [min, max]
                if len(shear) == 2:
                    self.shear = [shear[0], shear[1], 0., 0.]
                    # print("test shear 2", self.shear)
                elif len(shear) == 4:
                    self.shear = [s for s in shear]
                    # print("test shear 1", self.shear)
        else:
            self.shear = shear
